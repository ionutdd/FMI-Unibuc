...
substaexp (Qid s) target exp
    | s == target = exp
    | otherwise = Qid s
substaexp (PlusE e1 e2) target exp = PlusE (substaexp e1 target exp) (substaexp e2 target exp)
substaexp (TimesE e1 e2) target exp = TimesE (substaexp e1 target exp) (substaexp e2 target exp)
substaexp (DivE e1 e2) target exp = DivE (substaexp e1 target exp) (substaexp e2 target exp)
...
valueassn (BEX b) _ = b
valueassn (LEX e1 e2) state = (value e1 state) <= (value e2 state)
valueassn (NotEX a) state = not (valueassn a state)
valueassn (AndEX a1 a2) state = (valueassn a1 state) && (valueassn a2 state)
valueassn (DisjInfX an) state = any (\a -> valueassn a state) an
...
substassn (LEX e1 e2) target exp = LEX (substaexp e1 target exp) (substaexp e2 target exp)
substassn (NotEX a) target exp = NotEX (substassn a target exp)
substassn (AndEX a1 a2) target exp = AndEX (substassn a1 target exp) (substassn a2 target exp)
substassn (DisjInfX as) target exp = DisjInfX (map (\a -> substassn a target exp) as)
...
wp :: Stmt -> Assn -> Assn
wp Skip q = q
wp (AtrE var expr) q = substassn q var expr
wp (Seq stmt1 stmt2) q = wp stmt1 (wp stmt2 q)
wp (IfE cond stmt1 stmt2) q =
    let wp1 = wp stmt1 q
        wp2 = wp stmt2 q
    in AndEX (AndEX (convassn cond) wp1) (AndEX (NotEX (convassn cond)) wp2)
wp (WhileE cond body) q = DisjInfX (AndEX (NotEX (convassn cond)) q : fmap (AndEX (convassn cond)) (extr (wp body (wp (WhileE cond body) q))))
