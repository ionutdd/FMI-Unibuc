...
varngl :: Goal -> [String]
varngl g = [x | x <- freshvarlist, notElem x (vargl g)] 
...
evaluatenc p (Goal ats) c = union2 (map (evaluatemix p c) (mixg ats))  --nush exact aici cum e 
...
evaluatenp p g = union2 (map (evaluatenc p g) p)   --nici aici
