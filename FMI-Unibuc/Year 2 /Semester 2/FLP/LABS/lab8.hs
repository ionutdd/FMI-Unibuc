...
annotate_aux (Var x) (xs) = (AVar x, xs)
annotate_aux (Lam str term) (y:xs) = let (a, b) = (annotate_aux term xs) in (ALam str y a, b) 
annotate_aux (App lbdterm1 lbdterm2) (xs) = let (a, b) = (annotate_aux lbdterm1 xs) 
                                                (c, d) = (annotate_aux lbdterm2 b)
                                            in ((AApp a c), d)
...
constraints_aux (AVar x) (b : fs) gamma = let Just sigma = lookup x gamma in ([Equ (Variable sigma) (Variable b)], fs)
constraints_aux (ALam x sigma m) (b : (m' : fs)) gamma = 
    let (es, fs') = constraints_aux m (m':fs) ((x, sigma):gamma) 
    in (Equ (Variable b) (FuncSym "arrow" [Variable sigma, Variable m']):es, fs')
constraints_aux (AApp m n) (b : (m' : (n' : fs))) gamma = let (es1, zs1) = constraints_aux m fs gamma
                                                              (es2, zs2) = constraints_aux n zs1 gamma
                                                          in (Equ (Variable m') (FuncSym "arrow" [Variable n', Variable b]) :es1 ++ es2, zs2)
